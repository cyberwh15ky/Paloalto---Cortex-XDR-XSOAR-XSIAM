...
| Endpoint Protection<br><sub>(端點防護)</sub> | Safeguard endpoints with device control, firewall, and disk encryption<br><sub>(透過裝置控制、防火牆及磁碟加密保護端點)</sub> | ✓ | ✓ | ✓ | - |
| Detection and Response<br><sub>(偵測與回應)</sub> | Pinpoint attacks with AI-driven analytics and coordinate response<br><sub>(利用 AI 驅動分析精準定位攻擊並協調回應)</sub> | - | ✓ | ✓ | ✓ |
| Add-on: Host Insights<br><sub>(加購：主機洞察)</sub> | Find vulnerabilities and sweep across endpoints to eradicate threats<br><sub>(發現漏洞並全面掃描端點以根除威脅)</sub> | - | ✓ | ✓ | - |
| Third-Party Security Events<br><sub>(第三方安全事件)</sub> | Send security events from other data sources<br><sub>(傳送來自其他數據源的安全事件)</sub> | - | ✓ | ✓ | ✓ |
| Third-Party Security Logs<br><sub>(第三方安全日誌)</sub> | Send raw logs from other data sources<br><sub>(傳送來自其他數據源的原始日誌)</sub> | - | - | - | ✓ |
| Network Trafﬁc Analysis<br><sub>(網路流量分析)</sub> | Syslog, Kafka, DB, CSV ﬁle, FTP, NetFlow, Windows events, Pathﬁnder<br><sub>(分析 Syslog、Kafka、NetFlow、Windows 事件等流量)</sub> | - | - | - | ✓ |
